...
export default class doctorRepository implements IdoctorRepository {
  ...
  async singleDoctor(userData: Partial<Doctor>): Promise<Doctor | null> {
    try {
      return await Doctors.findOne({ email: userData.email })
        .select("-password")
        .populate({ path: "expertise" });
    } catch (err) {
      throw err;
      ...
      while (currentDate <= end) {
        ...
        while (shiftStart < shiftEnd) {
          const shiftEndTime = new Date(
            shiftStart.getTime() + duration * 60000
          );

          if (shiftEndTime > shiftEnd) break;

          const shiftStartFormatted = shiftStart.toLocaleTimeString([], {
            hour: "2-digit",
            minute: "2-digit",
            hour12: true,
          });
          const shiftEndFormatted = shiftEndTime.toLocaleTimeString([], {
            hour: "2-digit",
            minute: "2-digit",
            hour12: true,
          });
        ...
        operations.push({
          updateOne: {
            filter: {
              ...
              date: new Date(currentDate.setUTCHours(0, 0, 0, 0)),
            },
            update: {
              doctorId: doctorObjectId,
              date: new Date(currentDate.setUTCHours(0, 0, 0, 0)),
  ...
  async allAppointments(doctorId: string): Promise<IBooking[] | null> {
    try {
      return await Booking.find({ doctorId: doctorId }).populate({
        path: "patientId",
      });
    } catch (err) {
      throw err;
