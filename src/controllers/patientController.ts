...
export default class patientController implements IpatientController {
  ...
  async signupPatient(req: Request, res: Response, next: NextFunction) {
    try {
      ...
      if (photo) {
        ...
        if (userData === null) {
          const existingUser = await this._verificationService.singlePatient(
            email
          );
          if (!existingUser?.is_Blocked) {
            ...
            return res
              ...
              .json({
                ...
                favourite_doctors: existingUser?.favourite_doctors,
                tokens,
              });
          } else {
          ...
          if (!userData?.is_Blocked) {
            return res.status(201).json({
              ...
              favourite_doctors: userData.favourite_doctors,
              tokens,
            });
          } else {
  ...
  async doctors(req: Request, res: Response, next: NextFunction) {
    try {
      ...
      const data = await this._patientService.doctors(
        query,
  ...
  async verifyPayment(req: Request, res: Response, next: NextFunction) {
    try {
      ...
      if (generatedSignature === razorpay_signature) {
        ...
        res
          .status(400)
          .json({ success: false, paid: false, message: "Invalid Signature" });
      }
    } catch (error) {
      res
        .status(500)
  ...
  async setFavourite(req: Request, res: Response, next: NextFunction) {
    try {
      ...
      const data = await this._patientService.setFavourite(
        id as string,
        doctorId as string,
        status as boolean
      );
      return res
        .status(200)
        .json({
          message: "Updated Favourite List",
          favourite_doctors: data?.favourite_doctors,
        });
    } catch (err) {
      return res.status(500).json({ message: "Internal Error" });
    }
  ...
  async postRating(req: Request, res: Response, next: NextFunction) {
    try {
      ...
      const data = await this._patientService.postRating(
        doctorId,
        patientId,
        rating
      );
