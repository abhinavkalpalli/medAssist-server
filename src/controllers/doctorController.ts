...
export default class doctorController implements IdoctorController{
    ...
    async signupDoctor(req: Request, res: Response, next: NextFunction) {
        try{
            const {name,email,address,phone,gender,state,pincode,country,password,photo,is_Verified,expertise,dateOfBirth,languageKnown,currentWorkingHospital}=req.body
            if(photo){
            ...
            let tokens=await generateJwt({_id:existingUser?._id as string,email:existingUser?.email})
                if(!existingUser?.is_Blocked){
                    return res.status(201).json({message:'GoolgeAuth',name:existingUser?.name,email:existingUser?.email,address:existingUser?.address,phone:existingUser?.phone,gender:existingUser?.gender,state:existingUser?.state,pincode:existingUser?.pincode,country:existingUser?.country,photo:existingUser?.photo,is_Verified:existingUser?.is_Verified,expertise:existingUser?.expertise,dateOfBirth:existingUser?.dateOfBirth,currentWorkingHospital:existingUser?.currentWorkingHospital,workingDays:existingUser?.workingDays,experienceYears:existingUser?.experienceYears,workingHospitalContact:existingUser?.workingHospitalContact,documents:existingUser?.documents,_id:existingUser?._id,is_Blocked:existingUser?.is_Blocked,Wallet:existingUser?.Wallet,WalletHistory:existingUser?.WalletHistory,tokens,education:existingUser?.education})
                }else{
                    return res.status(403).json({message:'User is blocked'})
                }
            }else{
                if(!userData.is_Blocked){
                    return res.status(201).json({message:'GoolgeAuth',name:userData?.name,email:userData?.email,address:userData?.address,phone:userData?.phone,gender:userData?.gender,state:userData?.state,pincode:userData?.pincode,country:userData?.country,photo:userData?.photo,is_Verified:userData?.is_Verified,expertise:userData?.expertise,dateOfBirth:userData?.dateOfBirth,currentWorkingHospital:userData?.currentWorkingHospital,workingDays:userData?.workingDays,experienceYears:userData?.experienceYears,workingHospitalContact:userData?.workingHospitalContact,documents:userData?.documents,_id:userData?._id,is_Blocked:userData?.is_Blocked,Wallet:userData?.Wallet,WalletHistory:userData.WalletHistory,education:userData.education,tokens})
                }else{
                ...
                const data={name,email,address,phone,gender,state,pincode,country,password:hashedPassword,photo,expertise,dateOfBirth,languageKnown,currentWorkingHospital}
                const userData=await this._doctorService.signupDoctor(data)
                if(userData===null){
    ...
    async editDoctor(req: Request, res: Response, next: NextFunction){
        try{ 
            const {name,email,photo,currentWorkingHospital,dateOfBirth,experienceYears,gender,phone,workingHospitalContact,expertise,education}=req.body
            const data= await this._doctorService.editDoctor({name,email,photo,currentWorkingHospital,dateOfBirth,experienceYears,gender,phone,workingHospitalContact,expertise,education})
            return res.status(200).json({message:'Profile Updated',name:data?.name,email:data?.email,photo:data?.photo,currentWorkingHospital:data?.currentWorkingHospital,dateOfBirth:data?.dateOfBirth,experienceYears:data?.experienceYears,gender:data?.gender,phone:data?.phone,workingHospitalContact:data?.workingHospitalContact,expertise:data?.expertise,documents:data?.documents,_id:data?._id,Wallet:data?.Wallet,WalletHistory:data?.WalletHistory,education:data?.education})
        }catch(err){
            throw err
        }
    }
    ...
    async slotUpdate(req: Request, res: Response, next: NextFunction) {
        try {
            ...
            const updateResults = await Promise.all(slotsArray.map(async (slotData: any) => {
                const { doctorId, date, startTime, endTime, duration, breakTime } = slotData;
                const slotsData = {
                    doctorId,
                    startDate: date,  
                    endDate: date,    
                ...
                try {
                    const data = await this._doctorService.slotUpdate(slotsData as ISlotData);
                    if (data) {
                        return { success: true };
                    } else {
            ...
            if (allSuccessful) {
                ...
                const errors = updateResults.filter(result => !result.success).map(result => result.error);
                return res.status(400).json({ message: 'Some slots failed to update', errors });
            }
        } catch (err) {
            next(err); 
        }
     ...
     async fetchSlots(req: Request, res: Response, next: NextFunction) {
        try{
            ...
            if(data){
                ...
                return res.status(200).json({message:'Slots are fetched',Slots:[]})
            }
        }catch(err){
            throw err
        }
    ...
    async postPrescription(req: Request, res: Response, next: NextFunction) {
        try{
            ...
            const data=await this._doctorService.postPrescription(id,prescriptions)
    ...
    async patientHistory(req: Request, res: Response, next: NextFunction) {
        try {
            ...
            const data=await this._doctorService.patientHistory(patientId as string,doctorId as string)
    ...
    async allAppointments(req: Request, res: Response, next: NextFunction) {
        try{
            const {doctorId}=req.query
            const data=await this._doctorService.allAppointments(doctorId as string)
            return res.status(200).json({messgage:'Appointments',appointments:data})
        }catch(err){
            throw err
        }
    ...
    async slotUpdateDay(req: Request, res: Response, next: NextFunction) {
        try{
            ...
            const data=await this._doctorService.slotUpdateDay(doctorId,date,slots)
