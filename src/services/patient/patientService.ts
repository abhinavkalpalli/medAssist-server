...
export default class patientService implements IpatientService{
    ...
    constructor(){
     ...
     async setFavourite(id: string, doctorId: string,status:boolean): Promise<Patient | null> {
        try{
         ...
         if (status) {
             if (data.favourite_doctors) {
                 if (!data.favourite_doctors.some(doc => doc.doctorId.toString() === doctorId)) {
             ...
             if (data.favourite_doctors) {
                 data.favourite_doctors = data.favourite_doctors.filter(doc => doc.doctorId.toString() !== doctorId);
             }
         }
     ...
     async postRating(doctorId: string, patientId: string, rating: number): Promise<Doctor | null> {
        try {
